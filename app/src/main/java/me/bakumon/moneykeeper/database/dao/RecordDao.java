...
import me.bakumon.moneykeeper.database.entity.RecordWithMember;
...
public interface RecordDao {
    ...
    //获取某段时间内指定成员的record
    @Transaction
    @Query("SELECT record.* from record LEFT JOIN Member ON record.member_id=Member.id WHERE (Member.id=:memberId AND time BETWEEN :from AND :to)ORDER BY money DESC, create_time DESC")
    Flowable<List<RecordWithMember>> getRecordWithMember(Date from, Date to, int memberId);
